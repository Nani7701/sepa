...
export class Upload {
    ...
    getBusinessId(id) {
        return new Promise((resolve,reject) => {
            ...
            let sql =`SELECT business_id FROM business_details WHERE applicant_id=${id}`;
...
//insert the documents into database
    ...
    getFileDetails(docId) {
        return new Promise((resolve,reject) => {
            ...
            let sql=`SELECT kyb_doc_type,kyb_doc_file_type,kyb_doc_base64 FROM kyb_business_docs WHERE business_id = ${docId}`;
            DbInstance.executeQuery(sql).then(result => {
                logger.info("sql query executed");
                resolve(result);
           }).catch(err => {
