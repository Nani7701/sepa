...
export class TypesofbusinessComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,private homeService: HomeService, private alert: NotificationService, private routerNavigate: Router,private application: ApplicationsComponent,private homecom :HomeComponent ) {
  ...
  addsendFieldValue(){
    ...
    if(this.sendPaycountryFieldArray.length <= 9) {
      ...
      this.sendPaycountryFieldArray.push(obj);
  ...
  addreceivedFieldValue(){
    ...
    if(this.countryFieldArray.length <= 9) {
      ...
      this.countryFieldArray.push(obj);
  ...
  deletereceiveFieldValue(index) {
    ...
    receivedPaymenttoFrom(){
      console.log(this.countryFieldArray)
this.sendPayTempl=false;
