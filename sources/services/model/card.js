...
export class Card {
  ...
  getAllCards(applicantId) {
    logger.info('initiated');
    return new Promise((resolve, reject) => {
      let sql = `select payment_cards_id,card_type,name_on_card,card_number,card_month,card_year,status,default_card from payment_cards where applicant_id = ${applicantId}`;
      dbInstance.executeQuery(sql)
        .then(result => {
          resolve(result);
        })
