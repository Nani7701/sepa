...
export class OTP {
	...
	getUserId(referenceValue) {
		return new Promise((resolve, reject) => {
			...
			var sql = `select user_id from otp_validator where user_id = '${referenceValue}' OR mobile_no = '${referenceValue}'`;
	...
	saveOTP(otpValue, emailNumber, mobileNumber, otpExpireTimeout) {
		return new Promise((resolve, reject) => {
			...
			var sql = `INSERT into otp_validator (otp , user_id , mobile_no , expired) values 
	...
	updateOTP(referenceValue, oneTimePasscode, currentDate) {
		return new Promise((resolve, reject) => {
			...
			var sql = `UPDATE otp_validator SET otp = '${oneTimePasscode}', otp_status = 1,created= '${currentDate}'  where user_id = '${referenceValue}' OR mobile_no = '${referenceValue}'`;
	...
	isValidOTP(otp, referenceValue) {
		return new Promise((resolve, reject) => {
			...
			var sql = `select user_id,otp,otp_status,expired,mobile_no,created from otp_validator where otp = '${otp}' AND (user_id = '${referenceValue}' OR mobile_no = '${referenceValue}')`;
	...
	updateOtpStatus(rows, status) {
		return new Promise((resolve, reject) => {
			...
			var sql = `UPDATE otp_validator SET otp_status = '${status}' where user_id = '${rows[0].user_id}' OR mobile_no = '${rows[0].mobile_no}'`;
