...
export class country {
	...
	getStatusCurrency(){
		...
		return new Promise((resolve,reject)=>{
			let sql =`select country_id, country_name, currency, currency_symbol  from country where status = 1 ORDER BY country_name ASC`;
			DbInstance.executeQuery(sql).then(result=>{
	...
	getDistinctCurrency(){
		...
		return new Promise((resolve,reject)=>{
			let sql =`SELECT country_id, country_name, currency, currency_symbol, country_flag_img_path
			from country GROUP BY currency ORDER by country_name`;
			DbInstance.executeQuery(sql).then(result => {
