...
export class UserModel {
  ...
  loginUser(email, table, role) {
    return new Promise(function (resolve, reject) {
      ...
      let sql = `select password,status from ${table} where user_id = '${email}' and role_id = ${role}`;
  ...
  checkInitialPayment(applicant_id) {
    return new Promise(function (resolve, reject) {
      ...
      let sql = `select transactions.transaction_id from transactions WHERE applicant_id = ${applicant_id}`;
  ...
  responseCreation(email, roleName) {
    return new Promise(function (resolve, reject) {
      ...
      let sql = `select a.applicant_id, a.account_type, c.email, c.first_name, c.gender, c.last_name, c.mobile,c.phone,
      ad.address_line1, ad.address_line2, ad.city, ad.country_id, ad.postal_code, ad.region, ad.town, cn.country_name, k.kyc_status
      from applicant a, contact c, address ad,country cn, kyc k 
      where a.applicant_id=c.applicant_id and a.applicant_id= ad.applicant_id and ad.country_id = cn.country_id
      and  a.account_type ='${roleName}' and a.applicant_id= k.applicant_id and c.email = '${email}'`;
      DbInstance.executeQuery(sql).then(result => {
  ...
  getPin(table, email) {
    return new Promise(function (resolve, reject) {
      let sql = `select passcode_pin,status from ${table} where user_id = '${email}'`;
      DbInstance.executeQuery(sql).then(res => {
        resolve(res);
      })
    })
