...
export class Kyc {
  ...
  getUserByApplicant(kycApplicantId) {
    return new Promise((resolve, reject) => {
      ...
      let sql = `SELECT a.applicant_id, a.account_type,
  ...
  getIndividualApplicant(business_id , contact_id)
  {
    return new Promise((resolve, reject) => {
      logger.info('Initialize  getIndividualApplicant()');
      let sql = `SELECT k.kyc_status, k.kyc_vendor_id as id,k. k.kyc_transaction_id as TransactionNumber, CONCAT(c.first_name," ",c.last_name) as fullName , c.dob as dateOfBirth
      FROM kyc k
      INNER JOIN contact c
      ON k.applicant_id = c.applicant_id AND c.applicant_id = ${applicantId}`
      ;
      DbInstance.executeQuery(sql).then(res => {
  ...
  checkUser(applicantId) {
    ...
    return new Promise(function (resolve, reject) {
      var sql = `SELECT kyc_vendor_id as id ,kyc_status, kyc_transaction_id as TransactionNumber FROM kyc WHERE applicant_id = ${applicantId}`;
  ...
  updateKycDetails(kycObj) {
    ...
    return new Promise(function (resolve, reject) {
      var sql = `UPDATE kyc SET kyc_transaction_id = '${kycObj.transactionNumber}' , kyc_vendor_id = '${kycObj.id}', pep_status = '${kycObj.pep_status}' ,sanctions_status = '${kycObj.sanctions_status}'  WHERE applicant_id = ${kycObj.applicantId}`;
  ...
  updateKycStatus(kycObj) {
    ...
    return new Promise(function (resolve, reject) {
      var sql = `UPDATE kyc SET kyc_status = '${kycObj.status}' WHERE  kyc_transaction_id = '${kycObj.transactionNumber}' AND kyc_vendor_id = '${kycObj.id}'`;
      DbInstance.executeQuery(sql).then(function (results) {
