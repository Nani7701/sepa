...
export const forgotPassword = async (req, res) => {
  try {
    ...
    if (userRequest.account_type == "" || userRequest.email == "") {
      ...
      var userInfo = await user.forgotPassword(userRequest.business_type, userRequest.email)
      ...
      if (userInfo.length == 0) {
        ...
        var info = await sendResetLink(userRequest.email, userRequest.account_type, user_info)
...
export const changePassword = async (req, res) => {
  try {
    ...
    const userRequest = new UserRequest(req);
    var password = user.changePassword(userRequest.business_type, userRequest.applicant_id)
    ...
    if (hashPassword.verify(req.body.oldPassword, password[0].password)) {
      ...
      var message = user.saveNewPassword(userRequest.business_type, userRequest.newPassword, userRequest.applicant_id)
...
export const resetPassword = async (req, res) => {
  try {
    ...
    const userRequest = new UserRequest(req);
    var message = await user.resetPassword(userRequest.business_type, userRequest.password, _decrypt(userRequest.id))
