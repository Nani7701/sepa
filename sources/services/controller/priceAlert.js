			...
			_.forEach(data, function (row) {
				currencyExchange(row.from_currency, row.to_currency, null).then(function (value) {
					...
					if (_.size(data) == i) {
						...
						response.send(ResponseHelper.buildSuccessResponse({ priceAlerts: data },langEngConfig.message.payment.get_alert_success, STATUS.SUCCESS));
					}
				})
			})
			//return response.send(ResponseHelper.buildSuccessResponse({ priceAlerts:results }, langEngConfig.message.payment.get_alert_success, STATUS.SUCCESS));
		} else {
 ...
 */
 export const deletePriceAlerts=(request,response) => {
	 ...
	 priceAlert.checkPriceAlertInfo(alertId,status).then(data => {
		 if(data.length > 0) {		
			priceAlert.deletePriceAlert(applicant_id,alertId,!status, timeStamp).then(results => {
				if(results.affectedRows > 0) {
