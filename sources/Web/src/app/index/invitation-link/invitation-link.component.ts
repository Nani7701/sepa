...
export class InvitationLinkComponent implements OnInit,OnDestroy {
  ...
  // storeAppIdforKYC(){
  //   this.homeService.storeAppIdforKYC().subscribe(response => {
  ...
  // }
    submitForIdentId(){
      this.subs.sink=this.homeService.Bus_SubmitForKYCInvitaion(this.applicantId,this.Token,this.api_access_key,this.client_auth,this.member_id).subscribe(res=>{
        if(res['status']==0){
          if(res['data']['status']==1){
        ...
        KYClinkToMobile(mobilePlatform){
          this.subs.sink=this.homeService.KYClinkToMobile(null,this.email,mobilePlatform,this.identId).subscribe(res=>{
          if(res['status']==0){
            if(res['data']['status']==0){
